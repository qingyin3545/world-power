...
if IsCiv5BNW and not GreatPeopleIcons then
	GreatPeopleIcons = {
		...
		SPECIALIST_DOCTOR = "[ICON_GREAT_DOCTOR]",
		...
		UNIT_GREAT_DOCTOR = "[ICON_GREAT_DOCTOR]",
...
do
	...
	YieldIcons.YIELD_ELECTRICITY = "[ICON_RES_ELECTRICITY]"----------------新增发电----------------
	YieldIcons.YIELD_TOURISM = "[ICON_TOURISM]"----------------新增魅力----------------
end
...
--local function GetBuildingYields( buildingID, buildingType, buildingClassID, activePlayer )
	...
	--tip = tip .. " " .. L( "TXT_KEY_PRODUCTION_BUILDING_DEFENSE_MOD", building.GlobalDefenseMod )
	--end

	 --新增 城市攻击范围: 
	if building.BombardRange ~= 0 then 
		tip = tip .. " " .. L("TXT_KEY_PEDIA_CITY_BOMBARDRANGE", building.BombardRange)
	end
	...
	insert( tips, L( "TXT_KEY_PEDIA_CONQUESTPROB", building.ConquestProb))
	end

	-- 新增不可占领:
	if (building.NeverCapture == true) then 
	insert( tips, L( "TXT_KEY_PEDIA_NEVER_CAPTURE"))
	end

	-- 新增复制单位:
	if (building.InstantMilitaryIncrease == true) then 
	insert( tips,L"TXT_KEY_IMI11")
	end
...
-- TODO GameInfo.Building_ResourceYieldModifiers( thisBuildingType ), ResourceType, YieldType, Yield
	...
	for resource in GameInfo.Resources() do
		...
		if tip ~= "" then
				insert( tips, L("TXT_KEY_PEDIA_GLOBAL_RESOURCES")..ResourceString( resource ) .. ":" .. tip )
		end
	end
	...
	for Improvement in GameInfo.Improvements() do
		...
		if tip ~= "" then
			insert( tips, L"TXT_KEY_PEDIA_GLOBAL_RESOURCES" ..ResourceColor(L(Improvement.Description)).. ":" .. tip )
		end
	end
...
--Building_FreeSpecialistCounts unused ?
	...
	for row in GameInfo.Building_DomainFreeExperiencesGlobal( thisBuildingType ) do
		item = GameInfo.Domains[ row.DomainType ]
		if item and (row.Experience or 0) ~= 0 then
			insert( tips, L("TXT_KEY_EXPERIENCE_DOMAIN_GLOBAL")..UnitColor( L(item.Description) ).." "..L( "TXT_KEY_EXPERIENCE_POPUP", row.Experience ) )
		end
	end
	...
	for row in GameInfo.Building_YieldFromOtherYield(thisBuildingType) do
		...
		if item and (row.InYieldValue or 0) > 0 then
			insert( tips,(L( "TXT_KEY_EVERY_PER")..row.InYieldValue..L(item.IconString).."+"..row.OutYieldValue..L(item2.IconString)  ) )
		end
	end
	...
	for row in GameInfo.Policy_BuildingClassYieldModifiers( thisBuildingClassType ) do
		...
		and  GameInfo.Policies[ row.PolicyType].Dummy~=1  ---新增
		then
	...
	if IsCiv5 then
		...
		if not IsCiv5Vanilla then
			...
			for _, buildingClassType in pairs( buildingClassTypes3 ) do
				...
				local enhancedBuilding = GetCivBuilding( activeCivilizationType, buildingClassType )
				if enhancedBuilding and #tip > 0 then
					insert( tips, "[ICON_BULLET]" ..L"TXT_KEY_SV_ICONS_GLOBAL".." ".. BuildingColor( L(enhancedBuilding.Description) ) .. tip )
				end
			end
	...
	--if #buildings2 > 0 then
		--insert( tips, L"TXT_KEY_PEDIA_REQ_BLDG_LABEL2" .. " " .. concat( buildings2, ", ") )
	--end
	...
	-- -------------------------新增建筑新的资源关联-- --------------------------------------------------------------------
   local resources = {}
   for row in GameInfo.Building_EmpireResourceAnds( thisBuildingType ) do
		resource = GameInfo.Resources[ row.ResourceType ]
		if resource then
			insert( resources, ResourceString( resource ) ) -- resource.IconString
		end
	...
	for row in GameInfo.Building_EmpireResourceOrs( thisBuildingType ) do
		resource = GameInfo.Resources[ row.ResourceType ]
		if resource then
			insert( resources, ResourceString( resource ) ) -- resource.IconString
		end
	...
	if #resources > 0 then
		insert( tips, L"TXT_KEY_PEDIA_LOCAL_RESRC_LABEL2" .. " " .. resources )
	end
...
function GetHelpTextForImprovement( improvementID )
   ...
   if (improvement.NearbyEnemyDamage or 0)~=0 then
		insert( tips, L("TXT_KEY_PEDIA_NEAR_ENEMY_DAMAGE",improvement.NearbyEnemyDamage ))
	end

   if (improvement.NearbyFriendHeal or 0)~=0 then
		insert( tips,  L("TXT_KEY_PEDIA_NEAR_OUR_HEAL",improvement.NearbyFriendHeal))
	end

	-- -----------------新增临近城市----------------------------------------------------------
	tip = GetYieldString( GameInfo.Improvement_AdjacentCityYields( thisImprovementType ) )
	if #tip>0 then
		insert( tips, L"TXT_KEY_PEDIA_IMADJCITY_LABEL"..tip)
	end
		-- -----------------新增道路加成----------------------------------------------------------
	--tip = GetYieldString( GameInfo.Improvement_RouteYieldChanges( thisImprovementType ) )
	--if #tip>0 then
		--insert( tips, L"TXT_KEY_PEDIA_IMADJCITY_LABEL"..tip)
	--end
	...
	if #tip>0 then
		insert( tips, L"TXT_KEY_ABLTY_COASTALAND_SIDE_STRING"..tip)
	end
	...
	if #tip>0 then
		insert( tips, L"TXT_KEY_ABLTY_RIVER_SIDE_STRING"..tip)
	end

	--for row in GameInfo.Improvement_RiverSideYields( thisImprovementType ) do
		--SetKey( items, (row.Yield or 0)~=0 and row.YieldType, format("%s %+i%s", items[ row.YieldType ] or "", L"TXT_KEY_ABLTY_RIVER_SIDE_STRING", row.Yield, YieldIcons[ row.YieldType ] or "?" ) )
	--end
	...
	if item and item.ID > 0  then
	insert( tips, L"TXT_KEY_PEDIA_IMPROVEMENT_OBSOLETE_TECH_LABEL" .. " " .. TechColor( L(item.Description) ) )
	end
...
function GetHelpTextForProcess( processID )
	...
	for row in GameInfo.Process_ProductionYields{ ProcessType = process.Type } do
		...
		if percent == 0 then
		elseif IsCivBE then
			insert( tips, L( "TXT_KEY_PROCESS_GENERIC_HELP", percent, yield.IconString or "?", yield.Description or "???" ) )
		else
